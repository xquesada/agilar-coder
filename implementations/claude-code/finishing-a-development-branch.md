...
description: Use when work on a branch or worktree is complete and you need to decide how to finish it — merge, push for PR, keep, or discard
---

# Finishing a Development Branch — Claude Code Implementation

> Canonical skill: `skills/finishing-a-development-branch/SKILL.md`
>
> This file adds Claude Code-specific tooling and workflow. The working agreement, 4 options, worktree cleanup rules, and red flags in the canonical skill apply without modification. Read that file first.
...
## Claude Code Branch Finishing Protocol

You have the Bash tool, the EnterWorktree tool context, and full git access. Use them to execute a clean branch ending.
...
git worktree remove .claude/worktrees/feature-branch-name 2>/dev/null
git worktree prune
```
...
## Integration with EnterWorktree

If you used Claude Code's **EnterWorktree** tool to create the worktree:

- The worktree is at `.claude/worktrees/<name>/`
- Claude Code will prompt for cleanup on session exit
- For Options 1 and 4, cleanup should happen before session exit
- For Option 2, the worktree stays until the PR is merged
- For Option 3, the worktree stays with the paused branch

After finishing via Options 1 or 4, you may need to switch the session back to the main repo directory:

```bash
# If finishing from within a worktree, navigate back to main repo
...
- Do not leave branches without an explicit decision. "I'll figure it out later" means the branch is abandoned.
