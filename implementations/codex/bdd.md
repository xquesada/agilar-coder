...
Read the PBI's acceptance criteria. Translate each criterion into one or more scenarios. Create `.feature` files in the project's feature directory (commonly `features/`, `test/features/`, or `spec/features/`).
...
Create or edit step definition files. Keep steps thin — delegate to application code, don't implement business logic in steps.

### 4. TDD Inner Loop

Follow the `tdd` skill instructions for implementing the underlying code. BDD scenarios are the outer loop (WHAT); TDD is the inner loop (HOW).
...
Run the full feature suite in terminal. Read the output completely before claiming success (per `verification` skill).
