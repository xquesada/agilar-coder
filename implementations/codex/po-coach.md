...
# PO Coach — Codex Implementation

> Canonical skill: `skills/po-coach/SKILL.md`
>
> This file adds Codex-specific workflow. The coaching techniques, anti-patterns, and principles in the canonical skill apply without modification. Read that file first.
...
| Operation | Command |
|-----------|---------|
| **List ready PBIs** | Run in terminal: `curl -s {base_url}` → parse JSON, filter items where `status == "ready"` and matching epic/project |
| **Read PBI details** | Run in terminal: `curl -s {base_url}` → find item by `id` in response, read `notes`, `acceptance_criteria`, `checklist` |
| **Mark in_progress** | Run in terminal: `curl -s -X PATCH {base_url}/{id} -H 'Content-Type: application/json' -d '{"status":"in_progress"}'` |
| **Mark done** | Run in terminal: `curl -s -X PATCH {base_url}/{id} -H 'Content-Type: application/json' -d '{"status":"done"}'` |
| **Read acceptance criteria** | Same as Read PBI details — look for `acceptance_criteria` field or AC written in `notes` |
| **Update checklist** | Run in terminal: `curl -s -X PATCH {base_url}/{id} -H 'Content-Type: application/json' -d '{"checklist":[...]}'` |
...
| Operation | How |
|-----------|-----|
| **List ready PBIs** | Read the file. Find items where `status: ready`. |
| **Read PBI details** | Find the item by `id` in the YAML. Read `notes`, `acceptance_criteria`, `checklist` fields. |
| **Mark in_progress** | Edit the file to change `status: ready` to `status: in_progress` for the item. |
| **Mark done** | Edit the file to change `status: in_progress` to `status: done` for the item. |
| **Read acceptance criteria** | Same as Read PBI details — look for `acceptance_criteria` field. |
| **Update checklist** | Edit the file to update `done: false` to `done: true` for completed checklist items. |
...
| Operation | Command |
|-----------|---------|
| **List ready PBIs** | Run in terminal: `gh issue list --label ready --json number,title,labels` |
| **Read PBI details** | Run in terminal: `gh issue view {number}` — body contains acceptance criteria and checklist |
| **Mark in_progress** | Run in terminal: `gh issue edit {number} --add-label in_progress --remove-label ready` |
| **Mark done** | Run in terminal: `gh issue close {number}` |
| **Read acceptance criteria** | Run in terminal: `gh issue view {number} --json body` — parse Given/When/Then blocks |
| **Update checklist** | Run in terminal: `gh issue edit {number} --body "..."` — update task list checkboxes (`- [x]`) |

### Adapter: MCP-Based Tools (Jira, Miro, Linear)

The CLAUDE.md section identifies the project/board. The MCP server exposes tool functions that map to the logical operations.

**Generic pattern:**

1. Read CLAUDE.md for project key / board ID / team identifier
2. Use the MCP tools available in your environment (they appear as callable functions)
...
The exact tool names depend on the MCP server. Discover available tools and map them.
...
| Operation | Command |
|-----------|---------|
| **List ready PBIs** | Run in terminal: `ls backlog/ready/pbi-*.md` |
| **Read PBI details** | Read `backlog/{status}/pbi-NNN-*.md` — the file contains description, AC, notes, checklist, and plan |
| **Mark in_progress** | Run in terminal: `mv backlog/ready/pbi-NNN-*.md backlog/in_progress/` |
| **Mark done** | Run in terminal: `mv backlog/in_progress/pbi-NNN-*.md backlog/done/` |
| **Create PBI** | Create `backlog/pbi-NNN-description.md` with `# PBI #NNN: Title` header |
