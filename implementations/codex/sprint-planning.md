...
description: Use when you have a spec or requirements for a multi-step task, before touching code. Creates a step-by-step plan in the PBI file.
---

# Sprint Planning — Codex Implementation

> Canonical skill: `skills/sprint-planning/SKILL.md`
>
> This file adds Codex-specific workflow. The process and conventions in the canonical skill apply without modification. Read that file first.

## Entering Plan Mode

When the human asks you to plan (or when a PBI is complex enough to warrant a plan):

1. **Read the PBI.** Understand the goal, acceptance criteria, and any design decisions already made.
2. **Explore the codebase.** Search for and read existing patterns, conventions, file structure, and test infrastructure. The plan must fit the codebase as it is, not as you imagine it.
...
5. **Save the plan.** Write the `## Plan` section into the PBI file.
...
Maintain a checklist in the PBI file as you write the plan. This helps you (and the human) track which tasks are defined and which need refinement:
...
The checklist in the PBI file is your working memory during planning and execution.
...
This tells the executor how to set up their environment before starting. Use `git worktree add` for worktree management.
...
Search for and read files to answer these. Do not guess.
...
- **Up to ~8 tasks:** Execute in the current session. Just say: "Plan ready. Want me to start?" Move the PBI to `backlog/ready/`, then follow the executing-plans skill.
- **9+ tasks:** Recommend a fresh session. Say: "This plan has N tasks. I recommend starting a fresh session to execute it — that way context won't be lost midway. The PBI file is at `backlog/ready/pbi-NNN-description.md`. Start a fresh session and tell it to build the next ready PBI."

Do not mention context windows or compaction to the user. Just make the recommendation.

### Multi-Agent Mode

Dispatch independent task groups to parallel agents in worktrees. Plans are queued in `backlog/ready/`. See `skills/parallel-agents/` and `skills/git-worktrees/`.
